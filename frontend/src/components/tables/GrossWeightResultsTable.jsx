...
function gwGlobalFilter(row, _columnId, filterValue) {
...
export function GrossWeightResultsTable({ data }) {
  ...
  const invalidRows = useMemo(() => data.filter((r) => r.status === 'invalid'), [data]);

  const exportCsv = () => {
    if (!invalidRows.length) return;
    const cols = [
      { header: 'Voucher No', accessor: (r) => r.voucherNo ?? '' },
      { header: 'Manual gross', accessor: (r) => r.manualGross ?? '' },
      { header: 'Auto gross', accessor: (r) => r.autoGross ?? '' },
      { header: 'Difference', accessor: (r) => r.difference ?? '' },
      { header: 'Status', accessor: (r) => r.status ?? '' },
      { header: 'Issues', accessor: (r) => (r.issues || []).join('; ') },
    ];
    exportRowsToCsv(`gross-weight-invalid-${Date.now()}.csv`, cols, invalidRows);
  };
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative max-w-md flex-1">
          ...
          <Input
            ...
            placeholder="Search voucher, weights, status, issues…"
            className="pl-10"
          />
        </div>
        <Button variant="secondary" size="md" onClick={exportCsv} disabled={!invalidRows.length}>
