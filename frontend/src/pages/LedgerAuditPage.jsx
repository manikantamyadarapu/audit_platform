...
/**
 * Shared rate & ledger audit workspace for Sales and Purchase.
 * Uses the same API, validation engine, filters, exports, and UI.
 *
 * @param {{ config: import('../config/ledgerAuditConfig').SALES_LEDGER_AUDIT_CONFIG }} props
 */
export default function LedgerAuditPage({ config }) {
  const {
    sessionKey,
    rateRulesReturnTo,
    productAveragesPath,
  ...
  }, [filteredRecords, activeFilter, exportFilteredColumns, exportFilePrefix]);
  ...
  }, [filteredRecords, activeFilter, exportFilteredColumns, exportFilePrefix, pdfTitle]);
            ...
            <div className="flex gap-3 rounded-xl border border-amber-200/80 bg-amber-50/80 px-4 py-3 text-sm text-amber-950 dark:border-amber-900/50 dark:bg-amber-950/30 dark:text-amber-100">
              ...
              <div>
                ...
                <p className="mt-1 text-amber-900/85 dark:text-amber-100/85">
                  Open{' '}
                  <Link to="/scrutiny/rate-rule-book" state={{ returnTo: rateRulesReturnTo }} className="font-semibold underline">
                    Gold and silver rates
                  </Link>{' '}
      ...
      <section>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <KpiCard
            ...
            accent="violet"
            interactive
            onClick={() => navigate(productAveragesPath)}
          />
        </div>
      </section>
