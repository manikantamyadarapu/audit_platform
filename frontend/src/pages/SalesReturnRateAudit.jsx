...
import { SALES_RETURN_AUDIT_CONFIG } from '../config/salesReturnAuditConfig';

export default function SalesReturnRateAudit({ config = SALES_RETURN_AUDIT_CONFIG }) {
  ...
  const runValidation = useCallback(async () => {
    if (!returnFile) {
      auditToastError(config.missingFileToast);
      return;
    }
    setLoading(true);
    try {
      ...
      auditToastSuccess(config.successToast);
    } catch (e) {
  ...
  const exportProductComparisonExcel = useCallback(async () => {
    ...
    try {
      if (activeFilter === 'higherReturnRate') {
        downloadRowsXlsx(
          `${exportPrefix}-higher-rate-${Date.now()}.xlsx`,
  ...
  const exportProductComparisonPdf = useCallback(() => {
    ...
    exportRowsToPdf(
      `${exportPrefix}-products-${tag}-${Date.now()}.pdf`,
      config.productPdfTitle,
      productExportColumns,
      filteredProductComparison
    );
    auditToastSuccess('PDF export downloaded');
  }, [filteredProductComparison, activeFilter, productExportColumns, exportPrefix, config.productPdfTitle]);
    ...
    try {
      ...
      if (result?.exportColumns?.length) {
        ...
        downloadAuditExceptionXlsx(
          exportExceptionRows,
          exceptionColumnOrder,
          `${exportPrefix}-exceptions-${tag}-${Date.now()}.xlsx`,
  ...
  const exportExceptionPdf = useCallback(() => {
    ...
    const tag = effectiveActiveFilter ? `filtered-${effectiveActiveFilter}` : 'all';
    exportRowsToPdf(
      `${exportPrefix}-exceptions-${tag}-${Date.now()}.pdf`,
      config.exceptionPdfTitle,
      exceptionExportColumns,
      exportExceptionRows
    );
    auditToastSuccess('PDF export downloaded');
  }, [exportExceptionRows, effectiveActiveFilter, exceptionExportColumns, exportPrefix, config.exceptionPdfTitle]);
  const exportExceptionCsv = useCallback(() => {
    ...
    const tag = effectiveActiveFilter ? `filtered-${effectiveActiveFilter}` : 'all';
    exportRowsToCsv(
      `${exportPrefix}-exceptions-${tag}-${Date.now()}.csv`,
      exceptionExportColumns,
      exportExceptionRows
    );
    auditToastSuccess('CSV export downloaded');
  }, [exportExceptionRows, effectiveActiveFilter, exceptionExportColumns, exportPrefix]);
  return (
