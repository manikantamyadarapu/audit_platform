...
function roleBadgeClass(role) {
  ...
  if (r === 'ADMIN' || r === 'SUPER_ADMIN') {
    return 'bg-emerald-100 text-emerald-800 ring-emerald-200/80 dark:bg-emerald-950/40 dark:text-emerald-300 dark:ring-emerald-800/60';
  }
  if (r === 'AUDITOR') {
...
function DetailRow({ icon: Icon, label, value }) {
  return (
    <div className="flex items-start gap-4 rounded-2xl border border-[var(--color-border-soft)] bg-[var(--color-surface-subtle)] px-5 py-4">
      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[var(--color-surface-elevated)] text-emerald-700 ring-1 ring-[var(--color-border-soft)] dark:text-emerald-400">
        <Icon className="h-5 w-5" strokeWidth={1.75} />
      </div>
      <div className="min-w-0 flex-1">
        <p className="text-xs font-medium text-[var(--color-text-muted)]">{label}</p>
        <p className="mt-1 break-words text-sm font-medium text-[var(--color-text-primary)]">{value ?? '—'}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="pb-6">
      ...
      {!loading && user ? (
        <div className="space-y-6">
          <Card className="overflow-hidden">
            <CardBody className="p-0">
              <div className="bg-gradient-to-br from-emerald-600/10 via-[var(--color-surface-elevated)] to-[var(--color-surface-subtle)] px-6 py-8 sm:px-8">
                <div className="flex flex-col items-center gap-5 sm:flex-row sm:items-center">
                  <div className="flex h-24 w-24 items-center justify-center rounded-full bg-emerald-600 text-2xl font-semibold text-white shadow-[0_18px_45px_rgba(5,150,105,0.35)] ring-4 ring-[var(--color-surface-elevated)]">
                    {initials}
                  </div>
                  <div className="flex-1 text-center sm:text-left">
                    <h3 className="text-xl font-semibold text-[var(--color-text-primary)]">{user.name}</h3>
                    <p className="mt-1 flex items-center justify-center gap-2 text-sm text-[var(--color-text-secondary)] sm:justify-start">
                      ...
                      <span
                        className={cn(
                          ...
                          user.isActive !== false
                            ...
                            : 'bg-rose-50 text-rose-800 ring-rose-200/80 dark:bg-rose-950/40 dark:text-rose-300 dark:ring-rose-800/60'
                        )}
                      >
          ...
          <Card>
            <CardHeader>
              <h3 className="text-base font-semibold text-[var(--color-text-primary)]">Account details</h3>
            </CardHeader>
            <CardBody>
          ...
          <Card>
            <CardHeader>
              <h3 className="text-base font-semibold text-[var(--color-text-primary)]">Workspace access</h3>
            </CardHeader>
            <CardBody>
        ...
        <Card>
          <CardBody>
            <p className="text-sm text-[var(--color-text-secondary)]">No profile data found. Please sign in again.</p>
            <Link
              to="/login"
