...
export async function restoreAuditSession(params) {
  ...
  const { data } = await apiClient.get('/api/audit-sessions/restore', {
    params: query,
    headers: authHeaders(),
  });
...
export async function saveAuditSessionRemote(payload) {
  ...
  const { data } = await apiClient.post('/api/audit-sessions/save', payload, {
    headers: authHeaders(),
  });
...
export async function clearAuditSessionRemote(params) {
  ...
  const { data } = await apiClient.delete('/api/audit-sessions/clear', {
    params: query,
    headers: authHeaders(),
  });
