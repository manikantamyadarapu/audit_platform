...
def check_negative_cash_balance(row: dict[str, Any]) -> bool:
    ...
    # Parse numeric value
    from app.engines.cash_ledger_engine.parsers.parser import parse_balance
...
def check_cash_payment_above_threshold(row: dict[str, Any]) -> bool:
    ...
    - Closing Balance
    - Balance c/f
    - Bank account / bank deposit (see is_bank_account)
    ...
    # Bank deposits are not cash payments to parties/vendors
    if is_bank_account(contra_account):
        return False
