...
def _subcategory_labels_match(left: str | None, right: str | None) -> bool:
    a = _norm_subcategory(left)
    b = _norm_subcategory(right)
    if not a or not b:
        return False
    if a == b:
        return True
...
def _collect_subcategory_rows(
    *,
    ...
    """Previous-year rows for one Rule Book subcategory (category tab + dedicated sheets)."""
    ...
    for sheet in dedicated_product_sheets or ():
        ...
        if loc[0] != category:
            continue
        if not _subcategory_labels_match(loc[1], subcategory):
            continue
        _add(sheet)
...
def try_subcategory_fallback(
    *,
    ...
    candidates: list[Mapping[str, Any]] = _collect_subcategory_rows(
