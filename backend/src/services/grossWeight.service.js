...
/**
 * Validate a Gross Weight workbook: forward to Python, persist the audit
 * run, and fire completion/failure notifications.
 *
 * @param {import('express').Request} req
 * @returns {Promise<{ data: object, auditRunId: number | null }>}
 */
async function validateGrossWeight(req) {
  ...
  if (user?.id) {
    auditNotification
      .notifyAuditCompleted(user.id, AUDIT_KEYS.GROSS_WEIGHT, file.originalname, data)
      .catch(() => {});
  }
 ...
 * @param {Error} err
 */
function notifyGrossWeightFailure(req, err) {
  if (!req.user?.id) return;
  auditNotification
    .notifyAuditFailed(req.user.id, AUDIT_KEYS.GROSS_WEIGHT, req.file?.originalname, err.message)
    .catch(() => {});
}
 ...
 */
async function exportInvalidGrossWeight(records, options = {}) {
  return pythonClient.postGrossWeightExportInvalid(records, options);
}

module.exports = { validateGrossWeight, notifyGrossWeightFailure, exportInvalidGrossWeight };
